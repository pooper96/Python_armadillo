...
from .constants import (BG, CARD, TEXT, TEXT_DIM, ACCENT, BORDER, R,
                        H1, H2, BODY, SMALL, TOAST_SECS, TOPBAR_H, BOTNAV_H, tr)
...
class Card(BoxLayout):
    ...
    def __init__(self, **kw):
        ...
        with self.canvas.before:
            if self.elevated:
                Color(0,0,0,.18); self._sh1 = RoundedRectangle(pos=(self.x, self.y-2), size=self.size, radius=[R]*4)
                Color(0,0,0,.10); self._sh2 = RoundedRectangle(pos=(self.x, self.y-1), size=self.size, radius=[R]*4)
            Color(*CARD); self._bg = RoundedRectangle(pos=self.pos, size=self.size, radius=[R]*4)
        self.bind(pos=self._sync_bg, size=self._sync_bg)
    def _sync_bg(self, *_):
        if hasattr(self, "_sh1"):
            self._sh1.pos = (self.x, self.y-2); self._sh1.size = self.size
            self._sh2.pos = (self.x, self.y-1); self._sh2.size = self.size
...
class SmoothProgress(Widget):
    ...
    def __init__(self, **kw):
        super().__init__(**kw)
        with self.canvas:
            Color(*ACCENT); self._fg = Rectangle(pos=self.pos, size=(0, self.height))
        with self.canvas.before:
            Color(*BORDER); self._border = Rectangle(pos=self.pos, size=self.size)
        self.bind(pos=self._sync, size=self._sync)
        Clock.schedule_interval(self._tick, 1/60)
    def _sync(self, *_):
        self._border.pos = self.pos; self._border.size = self.size
        self._fg.pos = self.pos; self._fg.size = (self.display * self.width, self.height)
    def _tick(self, dt):
        self.display += (self.value - self.display) * min(1, self.speed * dt)
        self._fg.size = (self.display * self.width, self.height)

class _Toast(Label): pass
...
class ToastManager:
    ...
    def show(cls, text: str):
        ...
        lab.size_hint = (None, None); lab.opacity = 0
        lab.padding = ('14dp', '10dp'); lab.base_size = BODY
        lab.texture_update()
        lab.size = (lab.texture_size[0] + 28, lab.texture_size[1] + 20)
        lab.pos_hint = {'center_x': .5}; lab.y = 24
        with lab.canvas.before:
            Color(0,0,0,.8); lab._bg = RoundedRectangle(pos=lab.pos, size=lab.size, radius=[12,12,12,12])
        ...
        Clock.schedule_once(lambda *_: cls._dismiss(lab), 2.0)
    @classmethod
